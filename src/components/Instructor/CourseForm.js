...
export const CourseForm = () => {
  ...
  const formHandler = (e) => {
    e.preventDefault();
    //perform validation
    if (title.trim() === "" || code.trim() === "" || creditHour.trim() === "") {
      setError(true);
      return;
    }
    setError(false);
    // send the data to the baackend

    // if no error then update the courses array
    ctx.addCourse({ name: title, code, creditHours: creditHour });
    // navigate to the instrutor dashbaord
    navigate("/instructor");
  };
